...
export class CreateAlbumComponent {
  ...
  async onSubmit(): Promise<void> {
    if (this.selectedSong.length !== 0) {
      for (let i = 0; i < this.selectedSong.length; i++) {
        ...
        if (this.songForm.valid && this.files[0] && this.files[1]) {
          ...
          try {
            const songData = await this.songService.uploadSong(this.file, formData).toPromise();
  
            console.log('Song uploaded successfully:', songData);
            this.uploadedSongs.push(songData.id);
    ...
    if (this.albumForm.valid && this.files[0] && this.files[1]) {
      ...
      try {
        const albumData = await this.albumService.uploadAlbum(this.files[0][0], formData, this.uploadedSongs).toPromise();
        console.log('Album uploaded successfully:', albumData);
      } catch (error) {
